...
export function Habit() {
  ...
  const fatchHabitis = async () => {
    try {
      ...
      Alert.alert(
        "Ops",
        "Não foi possível carregar as informações dos hábitps"
      );
    }
    setLoading(false);
  };
  ...
  const handleToggleHabit = async (habitId: string) => {
    try {
        ...
        if(completedHabits.includes(habitId)) {
            ...
            setCompletedHabits(prevSatate => [...prevSatate, habitId]);
        }
    } catch(err) {
        console.log(err)
        Alert.alert('Ops', 'Não foi possíovel atualizar o status do hábito.')
    }
  }
